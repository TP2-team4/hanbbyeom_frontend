...
export function SignupForm({ onSuccess }: Props) {
    ...
    if (form.step === "talkLevel") {
        const handleTalkLevelSubmit = (
        ...
        return (
            <form
                ...
                onSubmit={handleTalkLevelSubmit}
            >
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm text-body">
                        이 값이 매칭 조건의 기본값이 되고, 내 프로필에
                        표시돼요. 언제든 마이페이지에서 바꿀 수 있어요.
                    </p>
                </div>
                ...
                >
                    {OPTIONS.map((option) => (
                        <SelectableCard
                            ...
                            selected={form.defaultTalkLevel === option.value}
                            role="radio"
                            aria-checked={
                                form.defaultTalkLevel === option.value
                            }
                            onClick={() => form.selectTalkLevel(option.value)}
                        />
                    ))}
                </div>
